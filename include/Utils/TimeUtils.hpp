...
class TimeUtils
{
  ...
  public:
    ...
    static constexpr Int DaysFromCivil(Int y, u32 m, u32 d) noexcept
    {
        ...
        return era * 146'097 + static_cast<Int>(doe) - 719'468;
    }
    ...
    static constexpr std::tuple<Int, u32, u32> CivilFromDays(Int z) noexcept
    {
        ...
        static_assert(std::numeric_limits<Int>::digits >= 20, "This algorithm has not been ported to a 16 bit signed integer");
        z += 719'468;
        const Int era = (z >= 0 ? z : z - 146'096) / 146'097;
        const u32 doe = static_cast<u32>(z - era * 146'097);                     // [0, 146096]
        const u32 yoe = (doe - doe / 1460 + doe / 36'524 - doe / 146'096) / 365; // [0, 399]
    ...
    template <class Duration = std::chrono::seconds>
        requires IsChronoDurationV<Duration>
    static std::tm MakeUtcTm(std::chrono::time_point<SysClock, Duration> tp = std::chrono::time_point_cast<Duration>(SysClock::now()))
    {
        using namespace std;
    ...
    {
        using Days = std::chrono::duration<int, std::ratio<86'400>>;
