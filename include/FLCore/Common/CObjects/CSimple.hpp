...
struct CSimple : CObject
{
    ...
    IMPORT ~CSimple() override;                                           // 56
    IMPORT void open(Archetype::Root*) override;                          // 76
    IMPORT f32 get_physical_radius_r(Vector&) const override;             // 100
    IMPORT void unmake_physical() override;                               // 116
    IMPORT void beam_object(const Vector&, const Matrix&, bool) override; // 124
