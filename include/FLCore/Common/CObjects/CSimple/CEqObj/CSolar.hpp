...
struct CSolar : public CEqObj
{
  public:
    ...
    IMPORT ~CSolar() override;
    IMPORT void __stdcall set_velocity(long, const Vector&) override;
    IMPORT void __stdcall set_angular_velocity(long, const Vector&) override;
    IMPORT int update(f32, u32) override;
    IMPORT Vector get_velocity() const override;
    IMPORT Vector get_angular_velocity() const override;
    IMPORT f32 get_mass() const override;
    IMPORT void cache_physical_props() override;
    IMPORT void init_physics(const Vector&, const Vector&) override;
    IMPORT ObjActivateResult activate(bool, u32) override;
    IMPORT bool get_activate_state(st6::vector<bool, st6::allocator<bool>>&) override;
    IMPORT void flush_animations() override;
    IMPORT CEquip* alloc_equip(u16, Archetype::Equipment*, bool) override;
    ...
    void*
        dunnoListPtr; // 116 Pointer to two-directional list, individual size 0xC, so only 1 value being kept aside from the prev and next pointers
