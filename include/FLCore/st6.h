...
namespace st6 {
    ...
    void _Construct(_T1* _P, const _T2& _V) { new((void *)_P) _T1(_V); }
    ...
    bool operator==(const allocator<_Ty>&, const allocator<_U>&) { return (true); }

    template <class _Ty, class _U>
    bool operator!=(const allocator<_Ty>&, const allocator<_U>&) { return (false); }
    ...
    public:
        ...
        vector(_It _F, _It _L, const _A& _Al = _A())
            : allocator(_Al), _First(0), _Last(0), _End(0) { insert(begin(), _F, _L); }
        ...
        void swap(_Myt& _X) {
            if (allocator == _X.allocator) {
                st6::swap(_First, _X._First);
                st6::swap(_Last, _X._Last);
                st6::swap(_End, _X._End);
            }
            else {
                _Myt _Ts = *this;
                *this = _X, _X = _Ts;
    ...
    protected:
        ...
        void _Xran() const {
            _THROW2(std::out_of_range, "invalid vector<T> subscript");
        }
...
public:
        ...
        void swap(_Myt& _X)
                {st6::swap(key_compare, _X.key_compare);
                if (allocator == _X.allocator)
                        {st6::swap(_Head, _X._Head);
                        st6::swap(_Multi, _X._Multi);
                        st6::swap(_Size, _X._Size); }
                else
...
protected:
	...
	void _Splice(iterator _P, _Myt& _X, iterator _F, iterator _L)
		{if (allocator == _X.allocator)
			...
			_X.erase(_F, _L); }}
	void _Xran() const
		{_THROW2(out_of_range, "invalid list<T> subscript"); }
	_A allocator;
	_Nodeptr _Head;
	size_type _Size;
	};
...
private:
	...
	void _Xran() const
		{_THROW2(std::out_of_range, "invalid string<T> subscript"); }
	void _Xlen() const
		{_THROW2(std::out_of_range, "invalid string<T> subscript"); }
